...
import com.example.shareholders.fragment.Fragment_OptionalFundDisplay.lvFoudListAdapter;
...
public class DragListView extends ListView {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_DOWN) {
			...
			final ViewGroup itemView = (ViewGroup) this.getChildAt(dragPosition
			...
			View dragger = itemView.findViewById(R.id.iv_of_item_drag);
			// 如果在右边位置（拖拽图片左边的20px的右边区域）
			if (dragger != null && x > dragger.getLeft() && ManageFlag.state) {
				// 准备拖动
				// 初始化拖动时滚动变量
	...
	public void onDrop(int y) {
		...
		if (dragPosition > 0 && dragPosition < getAdapter().getCount()) {
			...
			lvFoudListAdapter adapter = (lvFoudListAdapter) getAdapter();
			HashMap<String, Object> dragItem = (HashMap<String, Object>) adapter
					.getItem(dragSrcPosition);
			// 删除原位置数据项
