...
@ContentView(R.layout.activity_change_nick_name)
public class ChangeNickNameActivity extends ActionBarActivity {
	@ViewInject(R.id.et_nickname)
	private com.example.shareholders.common.ClearEditText et_nickname;
	DbUtils dbUtils;
		...
		try {
			personalInformation = dbUtils.findById(PersonalInformation.class,
					RsSharedUtil.getString(getApplicationContext(),
							AppConfig.UUID));
			et_nickname.setText(personalInformation.getUserName());
	...
	@OnClick({ R.id.rl_return, R.id.tv_confirm, R.id.et_nickname })
	private void onClick(View v) {
		...
		case R.id.tv_confirm:
			if (et_nickname.getText().toString().trim() != "") {
				// 修改用户名
				personalInformation.setUserName(et_nickname.getText()
						.toString());
				try {
				...
				bundle.putString("nickname", et_nickname.getText().toString());
				intent.putExtras(bundle);
				ChangeNickNameActivity.this.setResult(Activity.RESULT_OK,
						intent);
				finish();
			} else {
